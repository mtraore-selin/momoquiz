...
export class PlayQuizComponent implements OnInit {
  ...
  constructor(
    private challengeServe: ChallengeService,
  ...
  async ngOnInit() {
    ...
    if (isJoinedChallege) {
      this.challengeServe.playChallenge(this.gamePin).subscribe(
        (res) => {
  ...
  onSelectFromSingleSelectQuestion(question, event) {
    console.log(
      'Single select question and answer by user',
      question._id,
      event.target.value
    );
    let value = event.target.value;
